...
function SearchPage({ client, onLogout, onFound }) {
  ...
  const search = async () => {
    ...
    try {
      // Fetch cargo
      // Filter by client company name for security
      const companyFilter = client?.company_name ? `&client_name=ilike.*${encodeURIComponent(client.company_name)}*` : "";
      const res = await fetch(
        `${SB_URL}/rest/v1/sc_cargos?airway_bill=eq.${encodeURIComponent(q)}${companyFilter}&select=*`,
                        ...
                        {isSim && hadProblems && (
                          <div style={{ marginTop:8, background:"rgba(255,193,7,.04)", border:"1px solid rgba(255,193,7,.15)", borderRadius:8, padding:"10px 12px" }}>
                            ...
                            {history.filter(h=>h.answer==="nao").map((h,idx)=>(
                              <div key={idx} style={{ fontSize:12, color:"#718096", marginBottom:4 }}>
...
export default function App() {
  ...
  const logout = () => {
    try { localStorage.removeItem("sc_client"); } catch {}
    setClient(null); setResult(null);
  };
